...
---
<!-- #BeginLibraryItem "/Library/head_orav.lbi" --><!-- #EndLibraryItem --><h1>V$LATCH </h1>  
...
<pre>Columns
   ...
   LEVEL#
   NAME
   GETS     (successful <i>willing to wait</i> requests for a latch)
   MISSES   (latch request failed)
   SLEEPS   (how many times a <i>willing to wait</i> process slept)
   IMMEDIATE_GETS
   IMMEDIATE_MISSES
   WAITERS_WOKEN
   WAITS_HOLDING_LATCH
   SPIN_GETS (latch obtained after spinning once or more)
   SLEEP1
   SLEEP2
   SLEEP3
   SLEEP4
 <i>  SLEEP5
   ...
   SLEEP11</i>
   WAIT_TIME</pre>
<p>Columns SLEEP5, SLEEP6,... SLEEP11 are present for compatibility
with previous versions of Oracle. No data are accumulated for these columns.</p>
<p><b>Related:</b></p>
