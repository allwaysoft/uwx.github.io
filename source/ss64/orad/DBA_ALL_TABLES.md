...
---
<!-- #BeginLibraryItem "/Library/head_orad.lbi" --><!-- #EndLibraryItem --><h1>DBA_ALL_TABLES </h1><p> Description of all object and relational tables in the database </p> 
...
<pre>Columns
   ...
   OBJECT_ID_TYPE
      If user-defined OID,then USER-DEFINED,
      else if system generated OID,then SYSTEM GENERATED
   TABLE_TYPE_OWNER
   ...
   DEPENDENCIES
      Should we keep track of row level dependencies?</pre>
<p><b>Example</b></p>
<p>Show All Tables:</p>
<p class="code">COLUMN owner FORMAT A10<br>
COLUMN table_name FORMAT A35<br>
COLUMN tablespace_name FORMAT A15</p>
<p class="code">select owner, table_name, tablespace_name <br>
from dba_all_tables <br>
where owner NOT IN('SYSTEM','DBSNMP', 'ORDSYS', 'OUTLN','SYS') <br>
order by owner, table_name, tablespace_name;</p>
<p>Script that will generate the  SQL  to count the no. of rows in every table:</p>
<pre>Select 'Select table_name, count(*) from ' ||owner|| '.' ||table_name|| ';' 
From dba_all_tables
Order By owner, table_name;</pre>
<p><b>Related:</b></p>
<pre> DBA_ALL_TABLES         <a href="ALL_ALL_TABLES.html">ALL_ALL_TABLES</a>       <a href="USER_ALL_TABLES.html"> USER_ALL_TABLES</a>
 ...
 <a href="DBA_CONSTRAINTS.html">DBA_CONSTRAINTS</a></pre>
<p>
...
<hr>
<div id="bl" class="footer"><a href="DBA_ALL_TABLES.html#"><img src="../images/top.png" width="30" height="22" alt="Back to the Top"></a></div>
